...
public class LatamInv implements InventoryHolder, PlatformService {
    ...
    private Consumer<Inventory> onCreatedInventory;
    ...
    /**
     * Create a new LatamInv with a custom size.
     *
     * @param size a multiple of 9 as the size of the inventory
     * @see Bukkit#createInventory(InventoryHolder, int)
     */
    public LatamInv(int size) {
        this(owner -> Bukkit.createInventory(owner, SlotHandler.fromSize(size)));
    }

    /**
     * Create a new LatamInv with a custom size and title.
     *
     * @param size  a multiple of 9 as the size of the inventory
     * @param title the title (name) of the inventory
     * @see Bukkit#createInventory(InventoryHolder, int, String)
     */
    public LatamInv(int size, String title) {
        this(owner -> Bukkit.createInventory(owner, SlotHandler.fromSize(size), ComponentRenderer.translate(title)));
    }

    /**
     * Create a new LatamInv with a custom type.
     *
     * @param type the type of the inventory
     * @see Bukkit#createInventory(InventoryHolder, InventoryType)
     */
    public LatamInv(InventoryType type) {
        this(owner -> Bukkit.createInventory(owner, type));
    }

    /**
     * Create a new LatamInv with a custom type and title.
     *
     * @param type  the type of the inventory
     * @param title the title of the inventory
     * @see Bukkit#createInventory(InventoryHolder, InventoryType, String)
     */
    public LatamInv(InventoryType type, String title) {
        this(owner -> Bukkit.createInventory(owner, type, ComponentRenderer.translate(title)));
    }

    public LatamInv(Function<LatamInv, Inventory> inventoryFunction) {
        Objects.requireNonNull(inventoryFunction, "inventoryFunction");
        Inventory inv = inventoryFunction.apply(this);

        if (inv.getHolder() != this) {
            throw new IllegalStateException("Inventory holder is not LatamInv, found: " + inv.getHolder());
        }
    ...
    protected void onInventoryCreated(Consumer<Inventory> onCreatedInventory) {
        this.onCreatedInventory = onCreatedInventory;
    }
    ...
    public void open(Player player) {
        if (onCreatedInventory != null) {
            onCreatedInventory.accept(inventory);
        }
