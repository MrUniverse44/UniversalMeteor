...
public class SpigotPlatformHandle implements PlatformHandle {

    private final JavaPlugin plugin;

    public SpigotPlatformHandle(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public List<ClassLoader> getFallbacks() {
        ClassLoader myCL = plugin.getClass().getClassLoader();

        List<ClassLoader> fallbacks = new ArrayList<>(Arrays.stream(Bukkit.getPluginManager().getPlugins())
        ...
        ClassLoader serverCL = org.bukkit.Bukkit.class.getClassLoader();
        if (serverCL != null) fallbacks.add(serverCL);
